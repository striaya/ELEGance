      ...
      <div class="admin-content">
        ...
        <div class="table-card">
          <table class="data-table">
            ...
            <tbody>
              <?php if (empty($orders)): ?>
                <tr>
                  <td colspan="8" style="text-align:center;padding:48px;color:var(--text-light)">Belum ada pesanan.</td>
                </tr>
              <?php else: ?>
        ...
        <?php if ($totalPages > 1): ?>
          <div class="pagination">
            <?php if ($page > 1): ?><a href="?halaman=<?= $page - 1 ?>"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="15 18 9 12 15 6" />
                </svg></a><?php endif; ?>
            <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
              <?php if ($i === $page): ?><span class="active"><?= $i ?></span><?php else: ?><a href="?halaman=<?= $i ?>"><?= $i ?></a><?php endif; ?>
            <?php endfor; ?>
            <?php if ($page < $totalPages): ?><a href="?halaman=<?= $page + 1 ?>"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
